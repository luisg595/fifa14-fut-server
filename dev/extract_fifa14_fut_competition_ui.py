#!/usr/bin/env python3
"""Read-only FIFA 14 patch archive scanner for Seasons/Tournaments frontend contracts.

This never edits patch.big/patch.bh. It walks the indexed patch records, decodes
FIFA 14 chunkzip payloads, recursively inspects BIG4/BIGF packages, and exports
only blobs containing Offline Seasons/Tournaments frontend symbols. The goal is
to correlate the PC ActionScript field names with the live HTTP parser trace
without guessing wire members or changing the retail UI.
"""
...
TARGETS = (
    b"futOfflineSeasonEntry",
    b"futSeasonReport",
    b"GetOfflineSeasonInfo",
    b"GetOfflineSeasonState",
    b"SeasonsDivisions",
    b"OFFLINE_DIVISIONS",
    b"THRESHOLD_RELEGATION",
    b"THRESHOLD_PROMOTION",
    b"THRESHOLD_CHAMPION",
    b"MAX_POINTS_PER_SEASON",
    b"MAINTAIN_COINS",
    b"MAINTAIN_PACK",
    b"MAINTENANCE_PACK_ID",
    b"MAINTAIN_ITEM",
    b"TITLE_COINS",
    b"TITLE_PACK",
    b"TROPHY_PACK_ID",
    ...
    b"PROMOTION_PACK_ID",
    b"PROMO_ITEM",
    b"EVENT_CARDS_TOURNAMENT_LIST_SUCCESS",
    b"EVENT_CARDS_TOURNAMENT_LIST_FAILURE",
    b"GetOfflineActiveTournamentId",
    b"GetOfflineTournamentInfo",
    b"FUT_OFFLINE_TOURNAMENT",
    b"TOURNAMENT_OFFLINE_TREE",
    b"fccTournamentTrophies",
...
SECONDARY_TARGETS = (
    ...
    b"PROJECTION_POINTS",
    b"DIVISION",
    b"MAX_SEASONS_POINTS",
    b"MAINTENANCE_POINTS",
    b"PROMOTION_POINTS",
    b"TROPHY_POINTS",
    b"ROUND",
    b"PRIZE",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read-only FIFA 14 FUT Seasons/Tournaments frontend archive scanner")
    ...
    result = {
        "schema": "fifa14-beta28-competition-static-trace",
        "mode": "read-only",
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="fifa14-competition-ui-") as tmp:
            root = Path(tmp)
            ...
            lines = [
                "FIFA 14 FUT Seasons/Tournaments frontend static trace",
            ...
            if not collector.matches:
                lines.append("No strong Seasons/Tournaments frontend strings were found in patch.big. The live competition parser trace is still useful.")
        ...
        output.parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(prefix="fifa14-competition-ui-error-") as tmp:
            root = Path(tmp)
            result["fatal_error"] = str(exc)
            (root / "manifest.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
            (root / "summary.txt").write_text(f"Competition UI static scan could not run: {exc}\n", encoding="utf-8")
            write_zip(root, output)
